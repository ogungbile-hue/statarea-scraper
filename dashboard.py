...
<body class="min-h-screen flex flex-col antialiased selection:bg-brand-500 selection:text-white">
  ...
  <header class="sticky top-0 z-50 glass-card border-b border-dark-border px-6 py-3.5">
    <div class="max-w-7xl mx-auto flex items-center justify-between">
      <div class="flex items-center space-x-3.5">
        <!-- OFFICIAL ANIMATED EIGHTY-TWO BADGE -->
        <div class="w-11 h-11 relative flex items-center justify-center filter drop-shadow-[0_0_12px_rgba(255,107,53,0.35)]">
          <svg width="44" height="44" viewBox="0 0 500 500" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Eighty-Two (82) element badge" style="display: block; overflow: visible;">
        ...
        <div>
          <div class="flex items-center space-x-2">
            <h1 class="text-xl font-extrabold tracking-tight bg-gradient-to-r from-white via-orange-100 to-brand-500 bg-clip-text text-transparent">
              Onítẹ́tẹ́
            </h1>
          ...
          <p class="text-xs text-gray-400 font-medium">Daily High-Safety Predictions & Performance Analytics</p>
        </div>
      </div>

      <!-- NAVIGATION TABS -->
      <div class="flex items-center space-x-2 bg-dark-surface p-1 rounded-xl border border-dark-border">
        <button id="tabPredictionsBtn" onclick="switchTab('predictions')" class="px-4 py-1.5 rounded-lg text-xs font-bold transition text-white bg-brand-500 shadow-md">
          <i class="fa-solid fa-ticket mr-1.5"></i> Daily Slips
        </button>
        <button id="tabAnalyticsBtn" onclick="switchTab('analytics')" class="px-4 py-1.5 rounded-lg text-xs font-bold transition text-gray-400 hover:text-white">
          <i class="fa-solid fa-chart-pie mr-1.5"></i> Analytics & P&L
        </button>
      </div>

      <div class="flex items-center space-x-3">
        <!-- Live Scores Update Trigger -->
  ...
  <main class="flex-1 max-w-7xl w-full mx-auto px-6 py-8 space-y-8">
    ...
    <div id="tabPredictions" class="space-y-8">
      <!-- STATS OVERVIEW CARDS -->
      <div class="grid grid-cols-1 md:grid-cols-4 gap-5">
        <div class="glass-card rounded-2xl p-5 border border-dark-border">
          <div class="flex items-center justify-between mb-2">
            <span class="text-xs font-semibold text-gray-400 uppercase tracking-wider">Today's Matches</span>
            <i class="fa-solid fa-calendar-check text-brand-500"></i>
          </div>
          <div id="statTotalFixtures" class="text-3xl font-extrabold text-white">53</div>
          <div class="text-xs text-gray-500 mt-1">Processed from Statarea</div>
        </div>

        <div class="glass-card rounded-2xl p-5 border border-dark-border">
          <div class="flex items-center justify-between mb-2">
            <span class="text-xs font-semibold text-gray-400 uppercase tracking-wider">H2H Encounters</span>
            <i class="fa-solid fa-clock-rotate-left text-accent-500"></i>
          </div>
          <div id="statH2HCount" class="text-3xl font-extrabold text-white">688+</div>
          <div class="text-xs text-gray-500 mt-1">Filtered >= 2023 recency</div>
        </div>

        <div class="glass-card rounded-2xl p-5 border border-brand-500/30 glow-orange">
          <div class="flex items-center justify-between mb-2">
            <span class="text-xs font-semibold text-brand-400 uppercase tracking-wider">Banker Slip Odds</span>
            <i class="fa-solid fa-shield-halved text-brand-500"></i>
          </div>
          <div id="statBankerOdds" class="text-3xl font-extrabold text-brand-400">4.59x</div>
          <div id="statBankerLegs" class="text-xs text-brand-500/80 mt-1 font-semibold">6 Ultra-Safe Legs</div>
        </div>

        <div class="glass-card rounded-2xl p-5 border border-accent-500/30 glow-blue">
          <div class="flex items-center justify-between mb-2">
            <span class="text-xs font-semibold text-accent-400 uppercase tracking-wider">Value Slip Odds</span>
            <i class="fa-solid fa-chart-line text-accent-500"></i>
          </div>
          <div id="statValueOdds" class="text-3xl font-extrabold text-accent-400">4.55x</div>
          <div id="statValueLegs" class="text-xs text-accent-400/80 mt-1 font-semibold">6 Diversified Legs</div>
        </div>
      </div>
      ...
      <div class="space-y-4">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-sm text-gray-400">Conservative multi-market algorithms with dynamic H2H recency, cup rotation penalties, and double chance downgrades.</p>
          </div>
          <div class="flex items-center space-x-2">
            <button onclick="copySlipText()" class="px-3.5 py-2 rounded-xl bg-dark-surface hover:bg-gray-800 text-xs font-semibold text-gray-300 border border-dark-border transition flex items-center space-x-2">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6" id="slipsContainer">
          <!-- Banker Ticket Card -->
          <div class="glass-card rounded-2xl p-6 border border-brand-500/30 flex flex-col justify-between relative overflow-hidden">
            <div class="absolute top-0 right-0 px-4 py-1.5 bg-brand-500/20 border-b border-l border-brand-500/30 text-brand-400 text-xs font-bold rounded-bl-xl uppercase tracking-wider">
              Primary Banker
            </div>
            <div>
              <div class="flex items-center space-x-3 mb-3">
                <div class="w-9 h-9 rounded-xl bg-brand-500/20 text-brand-500 flex items-center justify-center font-bold border border-brand-500/30">
                  <i class="fa-solid fa-lock text-sm"></i>
                </div>
                <div>
                  <h3 class="text-lg font-bold text-white" id="bankerTitle">High-Safety 5-Odds Banker Slip</h3>
                  <p class="text-xs text-gray-400" id="bankerDesc">Ultra-conservative multi-market slip</p>
                </div>
              </div>

              <div class="flex items-center space-x-4 py-2 px-3 rounded-xl bg-dark-surface/80 border border-dark-border my-4">
                <div>
                  <span class="text-xs text-gray-400">Total Odds:</span>
                  <span class="text-base font-extrabold text-brand-400 ml-1" id="bankerTotalOdds">4.59x</span>
                </div>
                <div class="h-4 w-px bg-dark-border"></div>
                <div>
                  <span class="text-xs text-gray-400">Avg Confidence:</span>
                  <span class="text-base font-extrabold text-white ml-1" id="bankerAvgConf">73.0%</span>
                </div>
                <div class="h-4 w-px bg-dark-border"></div>
                <div>
                  <span class="text-xs text-gray-400">Total Legs:</span>
                  <span class="text-base font-extrabold text-gray-200 ml-1" id="bankerLegsCount">6</span>
                </div>
              </div>

              <!-- Legs List -->
              <div class="space-y-3 mt-4" id="bankerLegsList"></div>
            </div>
          </div>

          <!-- Value Ticket Card -->
          <div class="glass-card rounded-2xl p-6 border border-accent-500/30 flex flex-col justify-between relative overflow-hidden">
            <div class="absolute top-0 right-0 px-4 py-1.5 bg-accent-500/20 border-b border-l border-accent-500/30 text-accent-400 text-xs font-bold rounded-bl-xl uppercase tracking-wider">
              Diversified Value
            </div>
            <div>
              <div class="flex items-center space-x-3 mb-3">
                <div class="w-9 h-9 rounded-xl bg-accent-500/20 text-accent-400 flex items-center justify-center font-bold border border-accent-500/30">
                  <i class="fa-solid fa-scale-balanced text-sm"></i>
                </div>
                <div>
                  <h3 class="text-lg font-bold text-white" id="valueTitle">Conservative Multi-Market Value Slip</h3>
                  <p class="text-xs text-gray-400" id="valueDesc">Diversified low-risk ticket</p>
                </div>
              </div>
              ...
              <div class="flex items-center space-x-4 py-2 px-3 rounded-xl bg-dark-surface/80 border border-dark-border my-4">
                <div>
                  <span class="text-xs text-gray-400">Total Odds:</span>
                  <span class="text-base font-extrabold text-accent-400 ml-1" id="valueTotalOdds">4.55x</span>
                </div>
                <div class="h-4 w-px bg-dark-border"></div>
                <div>
                  <span class="text-xs text-gray-400">Avg Confidence:</span>
                  <span class="text-base font-extrabold text-white ml-1" id="valueAvgConf">72.9%</span>
                </div>
                <div class="h-4 w-px bg-dark-border"></div>
                <div>
                  <span class="text-xs text-gray-400">Total Legs:</span>
                  <span class="text-base font-extrabold text-gray-200 ml-1" id="valueLegsCount">6</span>
                </div>
              </div>

              <!-- Legs List -->
              <div class="space-y-3 mt-4" id="valueLegsList"></div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="glass-card rounded-2xl p-6 border border-dark-border space-y-5">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h3 class="text-xl font-bold text-white flex items-center space-x-2">
              <span>📊 Onítẹ́tẹ́ Match Explorer</span>
            </h3>
            <p class="text-xs text-gray-400">Search all daily fixtures with raw model coefficients and community voting</p>
          </div>

          <div class="flex items-center space-x-3">
            <div class="relative">
              <i class="fa-solid fa-magnifying-glass absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 text-xs"></i>
              <input type="text" id="searchInput" oninput="filterTable()" placeholder="Search team or league..." class="pl-9 pr-4 py-2 rounded-xl bg-dark-surface border border-dark-border text-sm text-gray-200 placeholder-gray-500 focus:outline-none focus:border-brand-500 transition w-64">
            </div>
            <select id="marketFilter" onchange="filterTable()" class="px-3 py-2 rounded-xl bg-dark-surface border border-dark-border text-sm text-gray-200 focus:outline-none focus:border-brand-500">
          ...
          <table class="w-full text-left text-sm text-gray-300">
            <thead class="bg-dark-surface/90 text-xs uppercase text-gray-400 font-semibold border-b border-dark-border">
              <tr>
                ...
                <th class="px-4 py-3 text-center">O 1.5</th>
                <th class="px-4 py-3 text-center">O 2.5</th>
                <th class="px-4 py-3 text-center">BTTS</th>
                <th class="px-4 py-3 text-center">Votes (1/X/2)</th>
              </tr>
            </thead>
    ...
    <div id="tabAnalytics" class="space-y-8 hidden">
      <!-- ANALYTICS KPI SUMMARY -->
      <div class="grid grid-cols-1 md:grid-cols-5 gap-5">
        <div class="glass-card rounded-2xl p-5 border border-brand-500/40 glow-orange">
          <div class="flex items-center justify-between mb-1.5">
            <span class="text-xs font-semibold text-brand-400 uppercase tracking-wider">Net Profit</span>
            <i class="fa-solid fa-coins text-brand-500"></i>
          </div>
          <div id="anNetProfit" class="text-3xl font-extrabold text-brand-400">+0.00u</div>
          <div class="text-xs text-gray-400 mt-1">1-unit flat stake baseline</div>
        </div>

        <div class="glass-card rounded-2xl p-5 border border-emerald-500/30">
          <div class="flex items-center justify-between mb-1.5">
            <span class="text-xs font-semibold text-emerald-400 uppercase tracking-wider">Ticket Win Rate</span>
            <i class="fa-solid fa-trophy text-emerald-500"></i>
          </div>
          <div id="anWinRate" class="text-3xl font-extrabold text-emerald-400">0.0%</div>
          <div id="anSettledRatio" class="text-xs text-gray-400 mt-1">0 / 0 settled slips</div>
        </div>

        <div class="glass-card rounded-2xl p-5 border border-accent-500/30">
          <div class="flex items-center justify-between mb-1.5">
            <span class="text-xs font-semibold text-accent-400 uppercase tracking-wider">Lifetime ROI</span>
            <i class="fa-solid fa-percent text-accent-500"></i>
          </div>
          <div id="anRoi" class="text-3xl font-extrabold text-accent-400">0.0%</div>
          <div class="text-xs text-gray-400 mt-1">Return on total staked</div>
        </div>

        <div class="glass-card rounded-2xl p-5 border border-dark-border">
          <div class="flex items-center justify-between mb-1.5">
            <span class="text-xs font-semibold text-gray-400 uppercase tracking-wider">Total Slips</span>
            <i class="fa-solid fa-receipt text-gray-400"></i>
          </div>
          <div id="anTotalSlips" class="text-3xl font-extrabold text-white">0</div>
          <div id="anPendingCount" class="text-xs text-amber-400 mt-1 font-semibold">0 in-play / pending</div>
        </div>

        <div class="glass-card rounded-2xl p-5 border border-dark-border">
          <div class="flex items-center justify-between mb-1.5">
            <span class="text-xs font-semibold text-gray-400 uppercase tracking-wider">Current Streak</span>
            <i class="fa-solid fa-fire text-brand-500"></i>
          </div>
          <div id="anCurrentStreak" class="text-2xl font-extrabold text-white">--</div>
          <div class="text-xs text-gray-400 mt-1">Consecutive outcomes</div>
        </div>
      </div>

      <!-- MARKET ACCURACY BREAKDOWN BARS -->
      <div class="glass-card rounded-2xl p-6 border border-dark-border space-y-4">
        <div class="flex items-center justify-between">
          <div>
            <h3 class="text-xl font-bold text-white flex items-center space-x-2">
              <span>🎯 Market-by-Market Accuracy Breakdown</span>
            </h3>
            <p class="text-xs text-gray-400">Hit rates across conservative betting market categories</p>
          </div>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-4 gap-4" id="marketAccuracyCards">
      ...
      <div class="glass-card rounded-2xl p-6 border border-dark-border space-y-5">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p class="text-xs text-gray-400">Complete record of daily, weekly, and monthly performance with match scores</p>
          </div>

          <!-- Timeframe Selector -->
          <div class="flex items-center space-x-1.5 bg-dark-surface p-1 rounded-xl border border-dark-border">
            <button id="tfSlipsBtn" onclick="switchTimeframe('slips')" class="px-3 py-1 rounded-lg text-xs font-bold text-white bg-brand-500 transition">Detailed Slips</button>
            <button id="tfDailyBtn" onclick="switchTimeframe('daily')" class="px-3 py-1 rounded-lg text-xs font-bold text-gray-400 hover:text-white transition">Daily</button>
            <button id="tfWeeklyBtn" onclick="switchTimeframe('weekly')" class="px-3 py-1 rounded-lg text-xs font-bold text-gray-400 hover:text-white transition">Weekly</button>
            <button id="tfMonthlyBtn" onclick="switchTimeframe('monthly')" class="px-3 py-1 rounded-lg text-xs font-bold text-gray-400 hover:text-white transition">Monthly</button>
          </div>
        </div>
  ...
  <script>
    ...
    function createLegCard(leg, theme) {
      ...
      if (leg.home_goals !== undefined && leg.home_goals !== null && leg.away_goals !== undefined && leg.away_goals !== null) {
        ...
        if (isWon) {
          scoreBadge = `<span class="px-2 py-0.5 rounded-md font-mono font-bold text-[11px] bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 flex items-center space-x-1"><i class="fa-solid fa-check text-[10px]"></i><span>${leg.home_goals} - ${leg.away_goals} (FT)</span></span>`;
        } else if (isLost) {
          scoreBadge = `<span class="px-2 py-0.5 rounded-md font-mono font-bold text-[11px] bg-rose-500/20 text-rose-400 border border-rose-500/30 flex items-center space-x-1"><i class="fa-solid fa-xmark text-[10px]"></i><span>${leg.home_goals} - ${leg.away_goals} (FT)</span></span>`;
        } else if (isLive) {
          scoreBadge = `<span class="px-2 py-0.5 rounded-md font-mono font-bold text-[11px] bg-amber-500/20 text-amber-400 border border-amber-500/30 flex items-center space-x-1 animate-pulse"><i class="fa-solid fa-circle-dot text-[10px]"></i><span>${leg.home_goals} - ${leg.away_goals} (LIVE)</span></span>`;
        } else {
          scoreBadge = `<span class="px-2 py-0.5 rounded-md font-mono font-semibold text-[11px] bg-gray-800 text-gray-300 border border-gray-700">${leg.home_goals} - ${leg.away_goals}</span>`;
        }
      } else {
        scoreBadge = `<span class="px-2 py-0.5 rounded-md font-mono text-[11px] bg-dark-bg text-gray-500 border border-dark-border/60">Scheduled</span>`;
      }

      return `
        <div class="p-3.5 rounded-xl bg-dark-surface/50 border border-dark-border hover:border-gray-700 transition">
          <div class="flex items-center justify-between mb-1.5">
            <div class="flex items-center space-x-2">
              <span class="text-xs font-mono font-semibold text-gray-400">${leg.time || '18:00'}</span>
              <span class="text-xs px-2 py-0.5 rounded-md ${badgeBg} border font-bold">${leg.market}</span>
              <span class="text-xs px-1.5 py-0.5 rounded bg-dark-surface border border-dark-border text-gray-400 text-[10px] uppercase font-semibold">${leg.risk_level || 'Safe'}</span>
            </div>
            <div class="flex items-center space-x-2">
              ${scoreBadge}
          ...
          <div class="flex items-center justify-between text-xs mt-2 pt-2 border-t border-dark-border/50">
